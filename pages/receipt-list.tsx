...
const ReceiptList = (props: ReceiptListProps) => {
    const { recordCount, isTemplates, onDelete } = props;
    const logsRef = db.collection('logs');
    const receiptsRef = db.collection('receipts');
    const receiptsQuery =
        isTemplates == true
            ? receiptsRef.where('isTemplate', '==', true)
            : receiptsRef.where('isTemplate', '==', false);
    const [receipts] = useCollectionData<Receipt>(receiptsQuery, {
        idField: 'id',
    });
    ...
    const getColumnSearchProps = (dataIndex: any) => {
        ...
        ({
            ...
            render: (text: any) =>
                useState(searchedColumn) === dataIndex ? (
                    <Highlighter
                        highlightStyle={{
    ...
    const columns = [
        ...
        },
        {
            ...
            key: 'issueDate',
            width: '10%',
            ...(getColumnSearchProps('issueDate') as any),
            render: (text: string, record: any) =>
                editingKey !== record.id ? (
                    <div style={{ textAlign: 'center' }}>
                    ...
                    <FormItem
                        ...
                        initialValue={moment(text, 'DD.MM.YYYY')}
                    >
        ...
        },
        {
            ...
            key: 'processDate',
            width: '10%',
            ...(getColumnSearchProps('processDate') as any),
            render: (text: string, record: any) =>
                editingKey !== record.id ? (
                    <div style={{ textAlign: 'center' }}>
                    ...
                    <div>
                        <Space size="middle">
                            <Button
                                ...
                                icon={<PlusOutlined />}
                                onClick={() => edit(record)}
                                style={{ width: 104, marginBottom: 10 }}
                            >
                                Edit
                            </Button>
                        </Space>
    ...
    const isRowChanged = (record: any, updatedData: any) => {
        return !(
            record.documentNumber === updatedData.documentNumber &&
    ...
    const whichRowsChanged = (record: any, updatedData: any) => {
        const changedRows = [];
        record.documentNumber !== updatedData.documentNumber
            ? changedRows.push('documentNumber')
            : null;
        record.issueDate !== updatedData.issueDate
            ? changedRows.push('issueDate')
            : null;
        record.processDate !== updatedData.processDate
            ? changedRows.push('processDate')
            : null;
        record.supplier !== updatedData.supplier
            ? changedRows.push('supplier')
            : null;
        record.vatAmount !== updatedData.vatAmount
            ? changedRows.push('vatAmount')
            : null;
        record.totalAmount !== updatedData.totalAmount
            ? changedRows.push('totalAmount')
            : null;
        return changedRows;
    };
        ...
        await form.validateFields().then(async () => {
            ...
            if (isRowChanged(record, updatedData)) {
                try {
                    ...
                    try {
                        const changedRows = whichRowsChanged(
                            record,
                            updatedData
                        );
                        let action: string = !record.isTemplate
                            ? ''
                            : '---Template---\n';
                        changedRows.forEach(
                            (changedRow: any, index: number) => {
                                const rowAction = `${
                                    index + 1
                                }: '${changedRow}' '${
                                    record[changedRow]
                                }'  is updated as '${
                                    updatedData[changedRow]
                                }';\n`;
                                action += rowAction;
                            }
                        );
    ...
    const showTotal = (totalItemCount: number) => {
                            ...
                            total:
                                receipts !== undefined
                                    ? recordCount
                                        ...
                                        : receipts.length
                                    : 0,
                            ...(getColumnSearchProps('documentNumber') as any),
                            showTotal,
                            defaultCurrent: 1,
